...
- **Nested htmx controls must be fully isolated from their ancestor request.** The MusicBrainz
  Search button inside the review apply form first inherited `hx-encoding="multipart/form-data"`
  (which dropped its GET query), then still inherited `hx-disabled-elt="find
  .review-submit-btn"`. HTMX resolves that relative selector from the button, finds no descendant,
  and aborts before issuing a request. The nested control uses `hx-disinherit="*"` and explicitly
  declares its GET parameters, include, target, swap, and `hx-disabled-elt="this"`; do not rely on
  selective disinheritance when the ancestor form gains new `hx-*` behavior.
