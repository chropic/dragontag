...
- **A new user-editable setting** → `config.UserSettings` field, `settings.html` form input (use the `hint(text)` macro for the inline description), and the `/settings` POST handler in `main.py`. The `timezone` field is a worked example: Docker `TZ` env (locked, always wins) → `settings().timezone` → `"UTC"`, resolved in `main._local_tz()`.
...
- **Linking a track to an existing album** → `tagging/partial.write_album_link_tags()` writes album/album-artist/disc-total/track-total/MB-album-id tags from an already-indexed library album onto a single track (no MusicBrainz network call); the UI is the "link to album" picker in `_track_edit_modal.html`, posting to `/library/tracks/{id}/link-album` in `main.py`. Title, artist, and track number are left untouched.
- **Global keyboard shortcuts** → the `dtKeys` IIFE in `base.html` owns universal keys (`/` focus search, `?` help overlay, `g`+letter navigation) and exposes `dtKeys.register(key, fn)` for page-specific bindings; `library.html`, `changes.html`, `queue.html`, `job_detail.html`, and `settings.html` each register their own keys to match their statusbar hints exactly. New pages with a statusbar key hint must wire it the same way.
